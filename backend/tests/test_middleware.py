...
async def test_protected_without_cookie_401():
    async with AsyncClient(transport=ASGITransport(app=_inner_app()), base_url="http://test") as ac:
...
async def test_valid_cookie_injects_headers_and_strips_spoof():
    ...
    async with AsyncClient(
        transport=ASGITransport(app=app), base_url="http://test", cookies={"cc_access": tok}
    ) as ac:
        # 客户端伪造的 X-User-ID 必须被剥掉
        r = await ac.get("/whoami", headers={"X-User-ID": "attacker", "X-User-Role": "user"})
...
async def test_garbage_token_401_without_code():
    ...
    async with AsyncClient(
        transport=ASGITransport(app=_inner_app()),
        base_url="http://test",
        cookies={"cc_access": "garbage.notavalidtoken"},
    ) as ac:
        r = await ac.get("/whoami")
    assert r.status_code == 401
    assert "code" not in r.json()
...
def _spa_inner_app():
    ...
    async def spa(request):
        return JSONResponse({"page": "spa", "uid": request.headers.get("X-User-ID")})
...
async def test_spa_entry_public_without_cookie():
    async with AsyncClient(
        transport=ASGITransport(app=_spa_inner_app()), base_url="http://test"
    ) as ac:
...
async def test_spa_public_strips_spoofed_user_header():
    ...
    async with AsyncClient(
        transport=ASGITransport(app=_spa_inner_app()),
        base_url="http://test",
