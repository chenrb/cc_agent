...
from backend.auth.routes import auth_router  # 仅作导入存在性
...
async def app_with_admin():
    ...
    async with SessionLocal() as s:
        s.add(
            models.User(
                username="admin", password_hash=hash_password("pw"), role="admin", is_active=True
            )
        )
        await s.commit()
    app = FastAPI()
...
async def test_login_success_sets_cookies(app_with_admin):
    ...
    assert r.json() == {"user": {"username": "admin", "role": "admin"}, "expires_in": 7200}
...
async def test_disabled_user_cannot_login(app_with_admin):
    async with SessionLocal() as s:
        u = (
            await s.execute(
                models.User.__table__.select().where(models.User.__table__.c.username == "admin")
            )
        ).one()
        await s.execute(
            models.User.__table__.update()
            .where(models.User.__table__.c.id == u.id)
...
async def test_logout_revokes_refresh_and_clears_cookies(app_with_admin):
    async with AsyncClient(
        ...
        # 响应删除两个 cookie
        names = {sc.split("=", 1)[0].strip() for sc in r.headers.get_list("set-cookie")}
...
async def test_login_cookie_secure_auto_http(app_with_admin, monkeypatch):
    ...
    monkeypatch.setattr(routes_mod, "resolve_cookie_secure", lambda scheme: scheme == "https")
    async with AsyncClient(
        transport=ASGITransport(app=app_with_admin), base_url="http://test"
    ) as ac:
        r = await ac.post("/auth/login", json={"username": "admin", "password": "pw"})
